<template>
  ...
  export default {
    ...
    methods: {
      async fetchUserData() {
        try {
          const response = await axios.get(`api/users/${this.uid}`); // 发起 GET 请求
          this.user = response.data; // 将返回的数据赋值给 user
          this.form.oldPassword = response.data.pwd; // 将用户的密码赋值给表单的旧密码
          this.identify = response.data.identify; // 获取用户单位
          if(this.identify==3){
            this.identify="学生"
          }else{
            this.identify="教师"
